...
#intersecting are all those combinations, so where the origins is not only one number(the actual polygon)
...
load("C:/Users/davidle.WISMAIN/Box/lab folder/hotspots/paper/raw datat/biome.int/btsrp.contr.RData")
...
ggplot(cont.df, aes(reorder(biome,value),value)) + 
  ...
  ggsave("boxplot_Gi.NORM(mean).alpha0.05.png", width = 8.8/1.3, height = 6.4/1.3)
